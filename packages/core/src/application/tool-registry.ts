...
export interface TurnArtifacts {
  ...
  /**
   * The hydrated products shown this turn, in the order the model saw them.
   *
   * ⚠️ THE IDS ALONE ARE NOT ENOUGH, and the reason is the whole architecture.
   * The widget renders a product card with a price on it. If `done` carried only
   * ids, that price would have to come from somewhere else — either a second
   * fetch, or by parsing it back out of the model's prose. Parsing it out of
   * prose means the number a customer sees was generated by a language model,
   * which is precisely what the two-plane rule exists to prevent.
   *
   * Carrying the hydrated Product here keeps price and availability on a typed
   * path from Shopify to the DOM, with no string in the middle.
   */
